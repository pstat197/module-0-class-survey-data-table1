...
We took the data from the PSTAT 197A capstone survey to explore any relationships or patterns that could help us better understand the students taking the capstone. Using statistical methods and R, we analyzed three research questions surrounding students’ project preferences, coding proficiency/comfortability, and research experience.

For the first analysis we examined if there were any factors that made students more likely to be indifferent towards/prefer a research capstone project. By examining p-values derived from chi-squared tests/ Fisher's exact type between our target variable (`projtype`) and each other variable, we found two plausible candidates, `updv.num` and `lang`. We wanted to know which specific responses were correlated to an indifference/preference for research, and ultimately found that the factor most contributing to a student's preference/indifference for reseatch was a student's who preference for R.

For the second, we examined whether taking CMPSC 9 or CMPSC 16 was related to students' self rated coding proficiency and comfortability. We found no statistical relationship between courses taken and either proficiency or comfortability. The correlation coefficients were near zero, and all p-values exceeded .05, suggesting no meaningful relationship between the class taken or coding ability.

For the third question, we found that students who participated in research and those who did not showed similar average numbers of domains of interest, with no statistically significant difference between their means (p = 0.6418). However, the research group exhibited greater variability and a more right-skewed, bell-shaped distribution with a peak at 3, while the non-research group exhibited a more uniform distribution.

## Data description

The dataset was obtained from a survey that all 2025 Fall Quarter PSTAT 197A students completed. Students were informed that this data would be used by the class for analysis and had the option to opt out of being in the dataset. Sixty students were surveyed, and fifty-two consented to have their data examined in the Module 0 project. Personal identifying information that were collected but not included in the dataset (removed to protect privacy) were time of submission, perm number, major(s), minor(s), standing, gender, race, goals for the project, and comments on research experience.


For each student, their self-rated programming, math, and statistics proficiency and comfort were recorded.

- Students had the option to rate their proficiency on a scale of beginner, intermediate, or advanced.

- Students had the option to rate their comfort from 1 to 5 (1 being the least comfortable).

Additionally, the dataset includes:

- Student’s number of upper-division courses taken (which can be either 0–2, 3–5, 6–8, or 9+).

- Whether they have a special domain of interest (yes, no, or unsure).

- Whether they have a preference for working on an industry project, a lab project, or no preference.

- Whether they have taken the following courses (1 for yes, 0 for no): 

  - PSTAT: 100, 115, 120, 122, 126, 127, 131, 134, 160, 174
  
  - CS: 5, 9, 16, 130
  
  - LING: 104, 110, 111

- Whether they have done research (true or false).

- Preferred coding language: Python, R, or No preference.
...
![](images/updivfreq.png)

![](images/PNG%20image.png){width="488"}
...
Language Response Frequency Table:
...
research_mean <- research %>% summarize(mean_interest_research = mean(interest_count))
...
# Calculate Mean # of domains of interest
no_research_mean <- no_research %>% summarize(mean_interest_no_research = mean(interest_count))

# Now let's look at the variance of these two! 
research_var <- research %>% summarize(var_interest_research = var(interest_count))
no_research_var <- no_research %>% summarize(var_interest_no_research = var(interest_count))

summary_table <- data.frame(
  Research_Participation = c("Participated", "Did not participate"),
  Mean_Domains = c(3.84375, 3.647059),
  Variance_Domains = c(2.587702, 1.617647),
  N = c(32, 17)
)

# Display with kable
kable(summary_table, caption = "Summary statistics of Domains of Interest by research participation")
...
```{r, echo=FALSE}
t_test_result <- t.test(research$interest_count, no_research$interest_count)
tidy_result <- broom::tidy(t_test_result)

kable(tidy_result, digits = 3, caption = "T-test Results")
```
